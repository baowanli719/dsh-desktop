...
const markPath = join(packageRoot, 'build', 'brand', 'logo.png')
const outputPath = join(packageRoot, 'build', 'app-icon.ico')

/**
 * Reuse the gs-worker brand logo for frames where the full shaded artwork
 * loses recognizable detail. The stable channel ships no vector mark, so
 * small native Windows chrome sizes fall back to the same brand artwork.
 * @returns {Promise<Buffer>} Brand artwork for small Windows frames.
 */
async function loadSmallFrameArtwork() {
  return readFile(markPath)
}
