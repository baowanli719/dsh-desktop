...
export function verifyWindowsPortable(
  ...
  const portablePath = join(
    options.desktopRoot,
    'dist',
    `gs-worker-${options.version}-x64-Portable.zip`,
  )
  ...
  const executable = entries.find(entry => entry.entryName.replaceAll('\\', '/') === 'gs-worker.exe')
  if (executable === undefined) {
    throw new Error(`Windows portable archive is missing gs-worker.exe: ${portablePath}`)
  }
