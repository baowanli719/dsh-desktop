...
$taskExistingInstalls = @(Get-ItemProperty $taskUninstallRoots -ErrorAction SilentlyContinue | Where-Object {
  $_.DisplayName -match '^gs-worker'
})

if ($taskExistingProcesses.Count -gt 0 -or $taskExistingInstalls.Count -gt 0) {
  throw 'Refusing to run while an existing gs-worker process or installation is present.'
}
...
$taskUserData = Join-Path $env:APPDATA 'gs-worker'
$taskDshHome = Join-Path $taskRoot 'dsh-home'
$taskActiveRunMarker = Join-Path $taskUserData 'crash-evidence\active-run.json'
$taskAppPath = Join-Path $taskInstallRoot 'gs-worker.exe'
$taskUninstallerPath = Join-Path $taskInstallRoot 'Uninstall gs-worker.exe'
if (Test-Path -LiteralPath $taskActiveRunMarker) {
  throw 'Refusing to overwrite an existing gs-worker active run marker.'
}
$taskResult = [ordered]@{
...
function Wait-TaskProcess([bool]$taskShouldExist, [int]$taskTimeoutSeconds = 30) {
  ...
  throw "Timed out waiting for gs-worker process state: shouldExist=$taskShouldExist"
}
...
try {
  ...
  $taskRemainingShortcuts = @($taskShortcutRoots | ForEach-Object {
    Get-ChildItem -LiteralPath $_ -Filter '*gs-worker*' -Recurse -ErrorAction SilentlyContinue
  })
