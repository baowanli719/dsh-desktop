 ...
 * @param bundleRoot - directory containing the built lib/client.js bundle.
...
export function verifyPackagedClientExternals(
  ...
  const bundlePath = join(bundleRoot, 'lib', 'client.js')
  let bundle: string
  try {
    bundle = readText(bundlePath)
  } catch (cause) {
    throw new Error(
      `dsh-plugin-desktop: packaged runtime at ${bundleRoot} is missing the browser client bundle lib/client.js`,
      { cause },
    )
  }
...
export function verifyPackagedRuntime(
  ...
  const packageRoot = context.packager.projectDir ?? DESKTOP_PACKAGE_ROOT
  ...
  verifyPackagedClientExternals(packageRoot, readText)
