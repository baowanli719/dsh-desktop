...
const COPY: Record<DesktopLocale, DesktopNativeCopy> = {
  en: {
    ...
    pluginRecoveryInstructions: 'Open DSH Terminal to update or remove the failing third-party plugin, then restart {brand}.',
    openTerminal: 'Open DSH Terminal',
    restart: 'Restart {brand}',
    dismiss: 'Dismiss',
    updateAvailableTitle: '{brand} Update Available',
    updateAvailableMessage: version => `{brand} ${version} is available.`,
    ...
    upToDateTitle: '{brand} Is Up to Date',
    upToDateMessage: 'No newer version of {brand} is available.',
    installedVersion: version => `Installed version: ${version}`,
    installerUnavailable: 'Installer downloads are unavailable in this build.',
    updateDownloadedTitle: '{brand} Update Downloaded',
    updateReady: version => `{brand} ${version} is ready to install.`,
    macInstallInstructions: 'The disk image has opened. Replace {brand} in Applications, then reopen it.',
    windowsInstallQuestion: 'Restart {brand} and run the installer now?',
    ...
    updateInstalled: version => `{brand} ${version} has been installed.`,
    ...
    terminalErrorMessage: '{brand} could not open a terminal.',
    diagnosticsErrorTitle: 'Unable to Export Diagnostics',
    diagnosticsErrorMessage: '{brand} could not export the diagnostic archive.',
  ...
  zh: {
    ...
    pluginRecoveryInstructions: '请打开 DSH 终端更新或移除失败的第三方插件，然后重启{brand}。',
    openTerminal: '打开 DSH 终端',
    restart: '重启{brand}',
    dismiss: '关闭',
    updateAvailableTitle: '{brand} 有可用更新',
    updateAvailableMessage: version => `{brand} ${version} 已可用。`,
    ...
    tryAgainLater: '请稍后重试。',
    upToDateTitle: '{brand} 已是最新版本',
    upToDateMessage: '当前没有更新版本的{brand}。',
    installedVersion: version => `当前版本：${version}`,
    installerUnavailable: '此构建不支持下载安装包。',
    updateDownloadedTitle: '{brand} 更新已下载',
    updateReady: version => `{brand} ${version} 已可安装。`,
    macInstallInstructions: '磁盘映像已打开。请替换“应用程序”中的{brand}，然后重新打开。',
    windowsInstallQuestion: '现在重启{brand} 并运行安装程序？',
...
/** Resolve the native dialog copy for one locale against the effective brand. */
export function desktopNativeCopy(locale: DesktopLocale, brand = currentBrand().name): DesktopNativeCopy {
