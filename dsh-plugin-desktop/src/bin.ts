/** Headless-safe npm launcher for the gs-worker Electron executable. */
...
Launch gs-worker with the selected Web-capable profile.
...
export function defaultDesktopUserDataDirectory(
  ...
  if (platform === 'win32') {
    ...
    if (appData === undefined || appData.length === 0) {
      throw new Error('APPDATA is unavailable; cannot locate gs-worker diagnostics')
    }
    return path.join(appData, 'gs-worker')
  }
  if (platform === 'darwin') return path.join(homeDirectory, 'Library', 'Application Support', 'gs-worker')
  const config = environment.XDG_CONFIG_HOME
  return path.join(config === undefined || config.length === 0 ? path.join(homeDirectory, '.config') : config, 'gs-worker')
}
...
async function launchElectron(): Promise<number> {
  ...
  try {
    ...
    process.stderr.write(
      ...
      + 'Or use the packaged gs-worker application.\n',
    )
    return 1
  }
