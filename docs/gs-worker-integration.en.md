...
### Vision model (local MCP bridge)

The server-side vision model (currently `gs-cloud/qwen36-35b`) is not pushed into the ClientConfig model list: the server deliberately keeps it out of the user-selectable list (gsclaw-server `scripts/fix-models-vision.ts`) and holds it only as the `models.visionModel` reference. Desktop agents reach it through a local MCP bridge: after Host bootstrap, `src/main.ts` mounts the upstream `@deepseek-ai/dsh-mcp-client` programmatically (stdio transport), launches `lib/mcp-vision-server.js` (`src/mcp-vision-server.ts`) under `ELECTRON_RUN_AS_NODE`, and registers the `mcp__vision__analyze_image` tool. The tool reads a local image (PNG/JPEG/WebP/GIF sniffed by magic bytes; raw file ≤ 2.5 MB — after base64 inflation the body must stay under both the loopback proxy's 4 MiB cap and the server express 4mb cap), calls the vision model as a multimodal chat completion through the loopback proxy at `POST /v1/gs-cloud/chat/completions`, and returns the text analysis. The per-boot proxy token reaches only this child via the mcp-client `config.env` carve-out — it still never enters `process.env` or disk; the server gateway's `llmProxy.resolveProxyTarget` separately allows the `visionModel` reference even though it is absent from the user-selectable whitelist. This lane is an MCP tool, not skill discovery, so it does not violate the local-skill ban.
