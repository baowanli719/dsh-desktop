...
**Lane two: dsh-vision-router (paste-based).** The community plugin ships as a desktop dependency mounted by the `vision-router` row in `cordis.patch.yml` (the row id matches the plugin's own bundle patch, so a manual `dsh plugin add` collapses instead of double-mounting). With the composer's "👁 Vision" control enabled, pasted images switch the turn to the `gs-cloud-vision` wrapper route and the plugin's fourteen `vision_*` tools handle multi-step work (describe/ground/crop/OCR). The vision backend is not static config: `mirrorGsLlmModelSettings` rewrites the `vision-router:` settings namespace's `httpProviders` with the per-boot loopback origin on every launch (`apiKeyEnv: DSH_DESKTOP_LLM_PROXY_TOKEN`, resolved through `ctx.credentials` → the launch-environment snapshot); the namespace is desktop-managed, so manual edits in the settings card are overwritten on the next boot. Shipped gates: `freeFallback: false` (images never reach the anonymous external OVH chain), `downscaleMaxPixels: 2000000` (inside both 4 MB proxy caps), and `deepseekTakeover: false` / `updateCheck: false` (provided by `patches/dsh-vision-router@2.1.3.patch`; ungated, the plugin would rebuild a direct `deepseek-official` route around the banned `llm-deepseek` row). Its dependency potrace is GPL-2.0: internal distribution only, registered in `THIRD_PARTY_NOTICES.md` under the same posture as the AGPL office plugin.
