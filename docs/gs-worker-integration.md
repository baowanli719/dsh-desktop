...
插件在 `agent/pre-step` 的每轮第一个步骤中，等待下游上下文注入完成后追加一条简短提醒，来源标记为 `dsh-plugin-desktop/prompt-language`。同一待提交消息批次去重；拒绝或空批次不添加提醒，后续工具步骤及结束探测不追加，因此不会因提醒而延长工具循环。插件卸载时，系统提示词段与事件监听一并移除。
