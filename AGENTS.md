...
- The server-side vision model (`gs-cloud/qwen36-35b`, held server-side as `models.visionModel` and deliberately absent from the user-selectable model list) reaches desktop agents through a local MCP bridge: `src/main.ts` mounts the upstream `@deepseek-ai/dsh-mcp-client` programmatically (stdio) after Host bootstrap, spawning `lib/mcp-vision-server.js` (`src/mcp-vision-server.ts`) under `ELECTRON_RUN_AS_NODE`, which registers `mcp__vision__analyze_image` and calls the vision model as a multimodal chat completion through the loopback proxy. The per-boot proxy token reaches only this child via the mcp-client `config.env` carve-out — keep it out of `process.env` and disk. The gsclaw-server LLM gateway (`llmProxy.resolveProxyTarget`) separately allows the `visionModel` reference outside the user-selectable whitelist; keep that allowance intact when changing the gateway. This is an MCP tool, not skill discovery, so the local-skill ban is unaffected.
