...
- The server-side vision model (`gs-cloud/qwen36-35b`, held server-side as `models.visionModel` and deliberately absent from the user-selectable model list) reaches desktop agents through two lanes, both riding the loopback proxy and the gsclaw-server gateway's `visionModel` allowance (`llmProxy.resolveProxyTarget`; keep that allowance intact when changing the gateway):
  ...
  - **dsh-vision-router (paste-based)**: the community plugin ships as a desktop dependency mounted by the `vision-router` row in `dsh-plugin-desktop/cordis.patch.yml` (the row id matches the plugin's own bundle patch, so `dsh plugin add` cannot double-mount it). Its vision backend is NOT static config: `mirrorGsLlmModelSettings` (`src/server/gs-llm-models.ts`) owns the `vision-router:` settings namespace outright and rewrites `httpProviders` with the per-boot loopback origin on every launch, with `apiKeyEnv: DSH_DESKTOP_LLM_PROXY_TOKEN` resolved through `ctx.credentials` → the launch-environment snapshot. Shipped gates: `freeFallback: false` (no anonymous external OVH chain), `downscaleMaxPixels: 2000000` (both proxy hops cap bodies at 4 MB), and `deepseekTakeover: false` / `updateCheck: false` — the last two are product switches added by `patches/dsh-vision-router@2.1.3.patch`; without the takeover gate the plugin would rebuild a direct `deepseek-official` route around the banned `llm-deepseek` row. Its dependency potrace is GPL-2.0: internal distribution only, tracked in `scripts/verify-licenses.mjs` and `THIRD_PARTY_NOTICES.md`. Neither lane is skill discovery, so the local-skill ban is unaffected.
